// src/automation/engine.js v2.6 — Chrome fantasma + extensão 2captcha
...
class AutomationEngine {
  ...
  async cleanup() { if(this.browser){try{
    // Se conectou via connect, desconecta sem fechar o Chrome
    if(this.browser.isConnected && this.browser.isConnected()) {
      await this.browser.disconnect();
    } else {
      await this.browser.close();
    }
  }catch{}this.browser=null;this.page=null;} }
  ...
  async initBrowser() {
    this.emit('starting',{message:'Conectando ao Chrome...'});
    const chromePort = process.env.CHROME_DEBUG_PORT || '9222';
    const chromeUrl = 'http://127.0.0.1:' + chromePort;

    // Tenta conectar no Chrome que já está rodando como serviço
    try {
      this.browser = await puppeteer.connect({
        browserURL: chromeUrl,
        defaultViewport: null
      });
      this.emit('starting',{message:'Conectado ao Chrome (porta '+chromePort+')'});
    } catch(e) {
      // Chrome não está rodando — tenta launch como fallback
      this.emit('starting',{message:'Chrome não encontrado na porta '+chromePort+', iniciando localmente...'});
    ...
    // Pega ou cria uma página
    const pages = await this.browser.pages();
    this.page = pages[0] || await this.browser.newPage();

    // Navega pra about:blank pra limpar qualquer página anterior
    ...
    try {
      ...
      // CDP pode falhar com connect, não é crítico
    }
  }
  ...
  async stepLogin(tipo,cred) {
    ...
    // Se Chrome serviço com extensão 2captcha: ela resolve automaticamente
    // Se Chrome local sem extensão: tenta via API do 2captcha
    this.emit('step',{message:'Clicando Continuar...'});
    await this.page.click('button#enter-account-id').catch(()=>{});

    // ── ETAPA 5: Aguarda captcha ser resolvido (extensão ou manual) ──
    // A extensão 2captcha resolve o desafio visual automaticamente em 30-60s
    // Se não tiver extensão, o usuário resolve pelo noVNC
    this.emit('step',{message:'Aguardando captcha (extensão 2captcha ou noVNC)...'});
